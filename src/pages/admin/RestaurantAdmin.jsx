...
import { fetchMenu, createMenuItem, updateMenuItem, deleteMenuItem, getImageUrl } from '../../services/api';
...
function RestaurantAdmin() {
  ...
  const openEditModal = (item) => {
    ...
    const previews = (item.images || []).map(img => getImageUrl(img));
          ...
          >
            <div className="card-image-strip">
              {item.images?.slice(0, 3).map((img, idx) => (
                <img key={idx} src={getImageUrl(img)} alt={item.name} className="strip-img" />
              ))}
              {item.images?.length > 3 && <span className="more-count">+{item.images.length-3}</span>}
              {!item.images?.length && <FiImage className="no-image-icon" />}
            </div>
